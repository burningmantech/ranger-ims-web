...
export const integers = function* ({ minValue, maxValue } = {}) {
  invariant(minValue >= 0, "minValue may not be less than 0");
  invariant(maxValue >= 0, "maxValue may not be less than 0");
  invariant(minValue <= maxValue, "minValue may not be greater than maxValue");

  while (true) {
    yield Math.floor(Math.random() * (maxValue - minValue)) + minValue;
  }
};
...
export const locationRadialHours = () => {
  return integers({ minValue: 1, maxValue: 12 });
};

export const locationRadialMinute = () => {
  return integers({ minValue: 0, maxValue: 59 });
};
