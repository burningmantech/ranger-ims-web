...
describe("NarrativeCard component: display", () => {
  test("rendered report entries, without system entries", async () => {
    const reportEntries = [
      new ReportEntry({
  ...
  test("rendered report entries, with system entries", async () => {
    const reportEntries = [
      new ReportEntry({
        created: DateTime.fromISO("2021-08-17T17:12:46.72Z").toUTC(),
        author: "bucket",
    ...
    await userEvent.click(screen.getByLabelText(/Show System Entries/));
    ...
    ).toEqual(reportEntries.map((e) => e.author));

    expect(
      Array.from(document.getElementsByClassName("report_entry_text")).map(
        (e) => e.textContent,
      ),
    ).toEqual(reportEntries.map((e) => e.text.replace(/[\n\r]/g, "")));

    expect(
      document.getElementsByClassName("report_entry_system").length,
    ).toEqual(reportEntries.filter((e) => e.systemEntry).length);
